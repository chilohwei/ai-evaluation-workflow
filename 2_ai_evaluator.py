...
请输出以下格式的JSON（注意：不要包含任何其他文字，只输出JSON）：
{
    ...
    def get_claude_review(self, content):
        ...
        prompt = f"""请作为一个专业的AI项目评审专家，对以下项目进行简短但有深度的点评。

要求：
1. 必须且只能输出一句话（以句号、感叹号或问号结尾）
2. 语气应该总体上表示肯定，同时可以指出一点不足（如果有）
3. 避免使用挖苦或讽刺的语气
4. 准确抓住项目最关键的特点或价值进行评价
5. 控制在50个字以内
6. 评论必须让人印象深刻、有见地
7. 特别关注项目的"人文精神"，即项目是否用心洞见和满足用户深层次需求，是否有文化价值和社会影响

评论风格指南：
- 使用积极肯定的语言，同时可以提出一个小的改进点
- 善用比喻突出项目的价值和潜力
- 指出项目如何解决用户痛点或创造价值
- 揭示项目背后的创新思维或人文关怀
- 用专业的方式点出项目的社会价值和人文意义

示例锐评：
1. "这款健康应用巧妙融合AI与用户需求，虽轻量却重实效，真正做到了技术为人服务而非喧宾夺主。"
2. "AI管家概念新颖且实用，将复杂技术简化为贴心服务，只是在隐私保护方面还需更深思考。"
3. "将AI引入睡眠领域既有温度又有科技感，是数字时代下对人类基本需求的优雅回应。"
4. "老年关怀与AI的结合点恰到好处，技术不再冰冷，而是成为情感连接的桥梁，令人感动。"
            ...
            try:
                ...
                data = {
                    "messages": [
                        {
                            "role": "system",
                            "content": "你是一个资深的AI项目评审专家，善于用生动、有个性的语言评价项目。你的评论既有专业深度，又有人文温度。你总是以积极肯定的态度为主，同时可以指出一点改进空间。"
                        },
                        {
                            "role": "user",
                            "content": prompt
